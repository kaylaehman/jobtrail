  ...
  return (
    <div className="space-y-2">
      ...
      {results && results.length === 0 && debounced.length >= 2 && !isLoading && (
        <div className="text-sm text-slate-400">No matches — try a different spelling.</div>
      )}
      {results && results.length > 0 && (
        <div className="rounded-md border border-slate-700 bg-slate-900 max-h-80 overflow-y-auto divide-y divide-slate-700">
          {results.map((c) => (
            <button
              key={c.qid}
              type="button"
            ...
            >
              ...
              <div className="text-[10px] text-slate-500 mt-0.5">{c.qid}</div>
            </button>
          ))}
        </div>
      )}
